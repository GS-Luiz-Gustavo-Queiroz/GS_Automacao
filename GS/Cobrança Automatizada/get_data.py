...
def get_data(server, user, password, database, dias):
    try:
        # Cria a conexão.
        ...
        cursor.execute(f"""
            ...
            CASE 
                WHEN LEN(SE1.E1_NFELETR) = 1 THEN CONCAT('00000000',TRIM(CAST(SE1.E1_NFELETR AS VARCHAR)))
                WHEN LEN(SE1.E1_NFELETR) = 2 THEN CONCAT('0000000',TRIM(CAST(SE1.E1_NFELETR AS VARCHAR))) 
                WHEN LEN(SE1.E1_NFELETR) = 3 THEN CONCAT('000000',TRIM(CAST(SE1.E1_NFELETR AS VARCHAR))) 
                WHEN LEN(SE1.E1_NFELETR) = 4 THEN CONCAT('00000',TRIM(CAST(SE1.E1_NFELETR AS VARCHAR))) 
                WHEN LEN(SE1.E1_NFELETR) = 5 THEN CONCAT('0000',TRIM(CAST(SE1.E1_NFELETR AS VARCHAR))) 
                WHEN LEN(SE1.E1_NFELETR) = 6 THEN CONCAT('000',TRIM(CAST(SE1.E1_NFELETR AS VARCHAR))) 
                WHEN LEN(SE1.E1_NFELETR) = 7 THEN CONCAT('00',TRIM(CAST(SE1.E1_NFELETR AS VARCHAR))) 
                WHEN LEN(SE1.E1_NFELETR) = 8 THEN CONCAT('0',TRIM(CAST(SE1.E1_NFELETR AS VARCHAR))) 
            ELSE SE1.E1_NFELETR END NUM_TITULO,
            SE1.E1_TIPO AS TIPO, 
            ...
            AND SE1.E1_VENCTO BETWEEN GETDATE()+{str(dias-1)} AND GETDATE()+{str(dias)}
            AND SE1.E1_BAIXA = ''
